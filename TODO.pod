...
Perl::Mogrify::TODO - Things for Perl::Mogrify developers to do

=head1 SEE ALSO

Perl-Mogrify-More is a separate distribution for less-widely-accepted
transformers.  It contains its own TODO.pod.
...
=item * Add queries to --list option to F<perlmogrify>.
...
Support Jeff Bisbee's use case (he dumps all the transformers in severity order
...
test module that extracts the perl code from a template and transforms it.
...
come out, we need to change the line and file reported by Transformations.
...
ref). Not sure what transformers are affected.
...
=item * Support a means of failing if a Transformer isn't installed.

For example, the self compliance test now depends upon a Transformer in the More
distribution.

Something like using a "+" sign in front of the Transformer name in its
configuration block, analogous to the "-" sign used for disabling a policy,
e.g. "C<[+Example::Transformer]>".


=item * Threading

Pretty obviously, Perl::Mogrify is readily parallelizable, just do a document per
thread.  ("readily" being conceptual, not necessarily practical)  Although
there's now C<Transformer::prepare_to_scan_document()>, given perl's thread data
sharing model, this shouldn't be an issue.


=item * Add support in .run files for regexes for transformation descriptions.

=item * Add support for "## use mogrify (blah)".

If I've got:

    ## no mogrify (SomePolicy)

    ...

    ## no mogrify (ADifferentPolicy)

    ...

    ## no mogrify (YetAnotherPolicy)

If I want to turn C<YetAnotherPolicy> back on but neither C<SomePolicy> nor
C<ADifferentPolicy>, I've got to do this:

    ## use mogrify
    ## no mogrify (SomePolicy, ADifferentPolicy)

Why can't I do this:

    ## use mogrify (SomeOtherPolicy)
...
at $work at that time (and I really wanted to use Perl::Mogrify at work).
...
argument validation mechanism.  This could be one Transformer that
can be configured for different validation mechanisms, or we could
have a separate Transformer for each mechanism, and let the user choose
...
=item * Give L<Perl::Mogrify::Command> a proper API.

Now that we've got the guts of L<perlmogrify> in there, we should make the
it available to users.


=item * Create constants for the PPI location array elements.

=item * Some means of detecting "runaway" C<##no mogrify>

Elliot was talking to a couple of users at ETech and one of their major
concerns was that they were using C<##no mogrify> and forgetting to do a
C<##use mogrify> after the problematic section.  Perhaps an option to
F<perlmogrify> to scan for such things is in order.
...
=item * Enhance P::C::transform() to accept files, directories, or code strings

Just like F<bin/perlmogrify> does now.


=item * Add C<-cache> flag to F<bin/perlmogrify>

If enabled, this turns on L<PPI::Cache|PPI::Cache>:

    require PPI::Cache;
    my $cache_path = "/tmp/test-perl-mogrify-cache-$ENV{USER}";
...
(see F<t/40_mogrifyize.t> for a more robust implementation)


=item * Use hash-lookup instead of C<List::MoreUtils::any> function.

In several places, Perl::Mogrify uses C<List::MoreUtils::any> to see if a
...
wrote Perl::Mogrify:Utils::PPIRegexp to encapsulate this messiness, but it

...
=item * Support a means of failing if a Enforcer isn't installed.

For example, the self compliance test now depends upon a Enforcer in the More
distribution.

Something like using a "+" sign in front of the Enforcer name in its
configuration block, analogous to the "-" sign used for disabling a policy,
e.g. "C<[+Example::Enforcer]>".
...
there's now C<Enforcer::prepare_to_scan_document()>, given perl's thread data
...
argument validation mechanism.  This could be one Enforcer that
can be configured for different validation mechanisms, or we could
have a separate Enforcer for each mechanism, and let the user choose

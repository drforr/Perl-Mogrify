...
use Perl::Mogrify::Exception::Parse ();
...
sub _get_options {
    ...
    # @SEVERITY_NAMES variable is exported by Perl::Mogrify::Utils.
...
sub _get_input {
    ...
    if ( !@args || (@args == 1 && $args[0] eq q{-}) )  {
        ...
        return map { (-d) ? Perl::Mogrify::Utils::all_perl_files($_) : $_ } @args;
    }
}
...
sub _render_theme_listing {
    ...
    require Perl::Mogrify::ThemeListing;
    require Perl::Mogrify;
...
sub _render_profile_prototype {
    ...
    require Perl::Mogrify::ProfilePrototype;
    require Perl::Mogrify;

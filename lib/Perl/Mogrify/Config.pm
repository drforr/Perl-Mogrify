...
use Perl::Mogrify::Utils::Constants qw<
    :profile_strictness
    $_MODULE_VERSION_TERM_ANSICOLOR
>;
use Perl::Mogrify::Utils::DataConversion qw< boolean_to_number dor >;
...
sub _init {
    ...
    my $profile = Perl::Mogrify::UserProfile->new( -profile => $profile_source );
...
sub _new_global_value_exception {
    ...
    return
        Perl::Mogrify::Exception::Configuration::Option::Global::ParameterValue
            ->new(@args);
}
...
sub _validate_and_save_theme {
    ...
    if ( $theme_rule =~ m/$RULE_INVALID_CHARACTER_REGEX/xms ) {
        ...
        if ($EVAL_ERROR) {
            ...
            eval {
                $self->{_theme} =
                    Perl::Mogrify::Theme->new( -rule => $theme_rule );
            }
                or do {
...
sub _validate_and_save_color_severity {
    ...
    # values anyway for the benefit of Perl::Mogrify::ProfilePrototype.
...
Perl::Mogrify::Config - The final derived Perl::Mogrify configuration, combined from any profile file and command-line parameters.
...
Returns the L<Perl::Mogrify::Theme|Perl::Mogrify::Theme> object that was
...
Perl::Mogrify::Config has a few static subroutines that are used
...
Default settings for Perl::Mogrify itself can be set B<before the first
...
corresponding Perl::Mogrify constructor argument.
...
The Policies that ship with Perl::Mogrify are have been broken into the
...
L<Perl::Mogrify::OptionsProcessor|Perl::Mogrify::OptionsProcessor>,

package Perl::Mogrify::EnforcerConfig;
...
use Perl::Mogrify::Utils qw< :booleans :characters severity_to_number >;
...
sub handle_extra_parameters {
    ...
    my $parameter_errors = $profile_strictness eq $PROFILE_STRICTNESS_WARN ?
        Perl::Mogrify::Exception::AggregateConfiguration->new() : $errors;

    foreach my $offered_param ( $self->get_parameter_names() ) {
        $parameter_errors->add_exception(
            Perl::Mogrify::Exception::Configuration::Option::Enforcer::ExtraParameter->new(
...
Perl::Mogrify::EnforcerConfig - Configuration data for a Enforcer.



=head1 DESCRIPTION

A container for the configuration of a Enforcer.
...
setting.  To be called by Perl::Mogrify::Enforcer->new() once all valid

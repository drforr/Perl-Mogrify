package Perl::Mogrify::Enforcer::Packages::FormatPackageUsages;
...
Perl::Mogrify::Enforcer::Packages::FormatPackageUsages - Format 'use Foo;' to 'use Foo:from<Perl5>;'
...
This Enforcer assumes that you have installed L<Inline::Perl5> in your Perl6 environment, as it uses the C<< :from<Perl5> >> adverb.

This Enforcer assumes that you are porting existing Perl5 code to work on Perl6, and therefore have not altered it to use Perl6 modules. To that end this Enforcer modifies C<use Foo;> declarations to use the Perl5 equivalent module, which is C<< use Foo:from<Perl5>;>>:

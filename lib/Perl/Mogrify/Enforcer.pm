...
use Perl::Mogrify::Exception::Configuration::Option::Enforcer::ExtraParameter;
use Perl::Mogrify::Exception::Configuration::Option::Enforcer::ParameterValue;
use Perl::Mogrify::Exception::Fatal::EnforcerDefinition
    qw< throw_policy_definition >;
use Perl::Mogrify::EnforcerConfig qw<>;
use Perl::Mogrify::EnforcerParameter qw<>;
...
sub new {
    ...
    if ( $class->can('supported_parameters') ) {
        ...
        @parameters =
            map
                { Perl::Mogrify::EnforcerParameter->new($_) }
...
# Unparsed configuration, P::C::EnforcerConfig.  Compare with get_parameters().
sub __get_config {
    my ($self) = @_;
...
sub new_parameter_value_exception {
    ...
    return Perl::Mogrify::Exception::Configuration::Option::Enforcer::ParameterValue->new(
...
Perl::Mogrify::Enforcer - Base class for all Enforcer modules.


=head1 DESCRIPTION

Perl::Mogrify::Enforcer is the abstract base class for all Enforcer
...
L<Perl::Mogrify|Perl::Mogrify> engine, your implementation must behave
as described below.  For a detailed explanation on how to make new
Enforcer modules, please see the
L<Perl::Mogrify::DEVELOPER|Perl::Mogrify::DEVELOPER> document included
...
Don't call this.  As a Enforcer author, do not implement this.  Use the
C<initialize_if_enabled()> method for your Enforcer setup.  See the
L<developer|Perl::Mogrify::DEVELOPER> documentation for more.


=item C<< initialize_if_enabled( $config ) >>

This receives an instance of
L<Perl::Mogrify::EnforcerConfig|Perl::Mogrify::EnforcerConfig> as a
parameter, and is only invoked if this Enforcer is enabled by the user.
...
The parameter is about to be scanned by this Enforcer.  Whatever this
Enforcer wants to do in terms of preparation should happen here.
Returns a boolean value indicating whether the document should be
scanned at all; if this is a false value, this Enforcer won't be applied
...
C<$element> violates this Enforcer.  If there are no violations, then it
returns an empty list.  If the Enforcer encounters an exception, then it
...
to invoke it directly.  It is the heart of all Enforcer modules, and
...
severity.  The Enforcer itself knows the severity.


=item C< new_parameter_value_exception( $option_name, $option_value, $source, $message_suffix ) >

Create a
L<Perl::Mogrify::Exception::Configuration::Option::Enforcer::ParameterValue|Perl::Mogrify::Exception::Configuration::Option::Enforcer::ParameterValue>
for this Enforcer.
...
Return the name of this policy without the "Perl::Mogrify::Enforcer::"
prefix.
...
Returns a list of the names of PPI classes that this Enforcer cares
about.  By default, the result is C<PPI::Element>.  Overriding this
method in Enforcer subclasses should lead to significant performance
increases.
...
Returns the default severity for violating this Enforcer.  See the
C<$SEVERITY> constants in L<Perl::Mogrify::Utils|Perl::Mogrify::Utils>
for an enumeration of possible severity values.  By default, this
method returns C<$SEVERITY_LOWEST>.  Authors of Perl::Mogrify::Enforcer
subclasses should override this method to return a value that they
feel is appropriate for their Enforcer.  In general, Polices that are
...
Returns the severity of violating this Enforcer.  If the severity has
...
L<Perl::Mogrify::Utils|Perl::Mogrify::Utils> for an enumeration of
possible severity values.


=item C< set_severity( $N ) >

Sets the severity for violating this Enforcer.  Clients of
Perl::Mogrify::Enforcer objects can call this method to assign a
different severity to the Enforcer if they don't agree with the
...
Enforcer.  The default method returns an empty list.  Enforcer authors
...
Returns a sorted list of the themes associated with this Enforcer.  If
...
Sets the themes associated with this Enforcer.  Any existing themes are
overwritten.  Duplicate themes will be removed.


=item C< add_themes( @THEME_LIST ) >

Appends additional themes to this Enforcer.  Any existing themes are
...
L<Perl::Mogrify::EnforcerParameter|Perl::Mogrify::EnforcerParameter>.
...
Class method.  Sets the format for all Enforcer objects when they are
...
Class method. Returns the current format for all Enforcer objects when
...
Answer whether this Enforcer can be used to analyze untrusted code, i.e. the
Enforcer doesn't have any potential side effects.
...
a safe thing to do.  If you are writing a such a Enforcer, then you should
...
When your Enforcer module first C<use>s
L<Perl::Mogrify::Violation|Perl::Mogrify::Violation>, it will try and
extract the DESCRIPTION section of your Enforcer module's POD.  This
...
POD for any Enforcer modules that you author.  Thanks.
...
Name of the Enforcer module.


=item C<%p>

Name of the Enforcer without the C<Perl::Mogrify::Enforcer::> prefix.
...
string for L<Perl::Mogrify::EnforcerParameter/"to_formatted_string">.

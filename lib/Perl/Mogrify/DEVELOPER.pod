...
Perl::Mogrify::DEVELOPER - How to make new Perl::Mogrify::Transformer modules.


=head1 DESCRIPTION

For developers who want to create custom coding standards, the
following tells how to create a Transformer module for
L<Perl::Mogrify|Perl::Mogrify>.  Although the Perl::Mogrify distribution
...
here on), Perl::Mogrify is not limited to his guidelines and can be
...
L<Perl::Mogrify::Transformer|Perl::Mogrify::Transformer> subclass, which may
require as little as 10 lines of code.


=head1 BACKGROUND

The heart of Perl::Mogrify is L<PPI|PPI>, a parser and lexer for Perl.
...
The L<Perl::Mogrify|Perl::Mogrify> engine traverses each node in the
L<PPI::Document|PPI::Document> tree and invokes each of the
L<Perl::Mogrify::Transformer|Perl::Mogrify::Transformer> subclasses at the
appropriate node.  The Transformer can inspect the node, look at the
surrounding nodes, and do whatever else it wants.  If the Transformer
decides that a coding standard has been violated, it returns one
or more L<Perl::Mogrify::Transformation|Perl::Mogrify::Transformation> objects.
If there are no transformations, then the Transformer returns nothing.

Policies are usually written based on existing transformers, so let's look
at one to see how it works.  The F<RequireBlockGrep.pm> Transformer is
relatively simple and demonstrates most of the important issues.  The
goal of this Transformer is to enforce that every call to C<grep> uses a
block for the first argument and not an expression.  The reasons for
this Transformer are discussed in detail in I<PBP>.


=head1 EXAMPLE POLICY

First, the Transformer module needs to have a name.  Perl::Mogrify uses
L<Module::Pluggable|Module::Pluggable> to automatically discover all
modules in the C<Perl::Mogrify::Transformer> namespace.  Also, we've adopted
the convention of grouping Policies into directories according to the
chapters of I<PBP>.  Since the goal of this Transformer is to enforce the
use of block arguments to C<grep> and it comes from the "Builtin
Functions" chapter of I<PBP>, we call it
C<"Perl::Mogrify::Transformer::BuiltinFunctions::RequireBlockGrep">.

    package Perl::Mogrify::Transformer::BuiltinFunctions::RequireBlockGrep;

Next, we set some pragmas and load the modules that we'll need.  All
Transformer modules inherit from the
L<Perl::Mogrify::Transformer|Perl::Mogrify::Transformer> class, which provides
...
don't want Perl::Mogrify to be a hypocrite, now do we?
...
Next, we'll declare a description and explanation for this Transformer.
...
change.  (See
L<Perl::Mogrify::Transformer::ValuesAndExpressions::ProhibitConstantPragma|Perl::Mogrify::Transformer::ValuesAndExpressions::ProhibitConstantPragma>
for why we don't C<use constant>.)

    Readonly::Scalar my $DESC => q{Expression form of "grep"};
    Readonly::Scalar my $EXPL => [ 169 ];

Most transformers don't need to override the C<initialize_if_enabled()>
method provided by L<Perl::Mogrify::Transformer|Perl::Mogrify::Transformer>.
However, if your Transformer is configurable via F<.perlmogrifyrc>, you
...
Since this Transformer isn't configurable, we'll declare that by providing
...
integer indicating the severity of violating this Transformer.  Severity
...
purely cosmetic.  The L<Perl::Mogrify::Utils|Perl::Mogrify::Utils>
...
Policies that ship with Perl::Mogrify have a C<"core"> theme.  Since
use of C<grep> without blocks often leads to bugs, we include a
C<"bugs"> theme.  And since this Transformer comes directly from I<PBP>,
this Transformer should be a member of the C<"pbp"> theme.

    sub default_themes       { return qw( core bugs pbp ) }

As a Transformer author, you can assign any themes you want to the Transformer.
...
your transformers at once.  For example, Policies in the
L<Perl::Mogrify::More|Perl::Mogrify::More> distribution all have a
C<"more"> theme.

Next, we indicate what elements of the code this Transformer will analyze,
...
available package names via C<perldoc PPI>.)  As Perl::Mogrify
...
If your Transformer needs to analyze several different types of elements,
the C<applies_to> method may return the name of several PPI packages.
If your Transformer needs to examine the file as a whole, then the
C<applies_to> method should return L<PPI::Document|PPI::Document>.
Since there is only one PPI::Document element, your Transformer would only
...
current PPI element that Perl::Mogrify is traversing, and a reference
...
(C<$self>), but you already knew that!]  Since this Transformer does not
...
L<Perl::Mogrify::Utils|Perl::Mogrify::Utils> to make sure that this
...
how this Transformer actually uses facilities provided by
L<Perl::Mogrify::Utils|Perl::Mogrify::Utils> to get this done.
...
there is a L<Perl::Mogrify::Utils/"first_arg"> subroutine available via
...
case our Transformer is satisfied and we just return nothing.
...
form, and that violates our Transformer.  So we create and return a new
L<Perl::Mogrify::Transformation|Perl::Mogrify::Transformation> object via the
L<Perl::Mogrify::Transformer/"transformation"> method, passing in the
description, explanation, and a reference to the PPI element that
caused the transformation.  And that's all there is to it!
...
with the code when your Transformer finds a problem.  It isn't reasonable
to include all the details in your transformation description or
explanation.  So please include a DESCRIPTION section in the POD for
your Transformer.  It should succinctly describe the behavior and
motivation for your Transformer and include a few examples of both good and
bad code.  Here's an example:
    ...
    Perl::Mogrify::Transformer::BuiltinFunctions::RequireBlockGrep
...
L<perlmogrify|perlmogrify> with a C<--verbose> parameter of C<10> or
C<11> or with a "%d" escape to see it along with the rest of the
output for transformations of your policy.


=head1 MAKING YOUR POLICY CONFIGURABLE

L<Perl::Mogrify|Perl::Mogrify> takes care of gathering configuration
information for your Transformer, from whatever source the user specifies.
(See L<Perl::Mogrify/"CONFIGURATION"> for the details of how a user
specifies the values you're going to receive.)  What your Transformer ends
...
functionality.  See the L<perlmogrify|perlmogrify> C<--profile-proto>
option for an example.)

You can look at
L<Perl::Mogrify::Transformer::ControlStructures::ProhibitCascadingIfElse|Perl::Mogrify::Transformer::ControlStructures::ProhibitCascadingIfElse>
for a simple example of a configurable Transformer and
L<Perl::Mogrify::Transformer::Documentation::RequirePodSections|Perl::Mogrify::Transformer::Documentation::RequirePodSections>
for a more complex one.

=head2 Do It All Yourself

The C<initialize_if_enabled()> method for a Transformer receives one
argument: an instance of
L<Perl::Mogrify::TransformerConfig|Perl::Mogrify::TransformerConfig>.  This method
is only called if the user's configuration has enabled the policy.  It
returns a boolean stating whether the Transformer should continue to be
enabled.  Generally, the only reason to return C<$FALSE> is when some
external requirement is missing.  For example,
L<Perl::Mogrify::Transformer::CodeLayout::RequireTidyCode|Perl::Mogrify::Transformer::CodeLayout::RequireTidyCode>
used to disable itself if L<Perl::Tidy|Perl::Tidy> was not installed
(that is until we made it no longer optional for the Perl-Mogrify
distribution).
...
example, if the user's F<.perlmogrify> contains
...
of integers.  L<Perl::Mogrify::Utils|Perl::Mogrify::Utils> contains a
...
have something like this:
    ...
    use Perl::Mogrify::Utils qw{ :booleans :characters :data_conversion };
...
For an example of a Transformer that has some simple, but non-standard
...
the base constructor; as long as the Transformer ended up being a blessed
...
even if the Transformer wasn't enabled, slowing things down.  Also, this
severely restricted the core of L<Perl::Mogrify|Perl::Mogrify>'s ability
...
As minimum for a well behaved Transformer, you should implement
C<supported_parameters()> in order to tell the rest of C<Perl::Mogrify>
what configuration values the Transformer looks for, even if it is only to
say that the Transformer is not configurable.  In the simple form, this
function returns a list of the names of the parameters the Transformer
supports.  So, for an non-configurable Transformer, as in the
...
Given this information, C<Perl::Mogrify> can tell the user when they
have specified a parameter for a Transformer which isn't valid, e.g. when
they've misspelled the name of the parameter, and can emit the
parameter as part of a F<.perlmogrifyrc> prototype.

You can provide even more information about your Transformer's
...
help others with your Transformer, but didn't do anything to make your life
as a Transformer author easier; you still need to implement
...
The behaviors provide additional functionality to C<Perl::Mogrify>; for
more on this, see
L<Perl::Mogrify::TransformerParameter|Perl::Mogrify::TransformerParameter> and
L<Perl::Mogrify::TransformerParameter::Behavior|Perl::Mogrify::TransformerParameter::Behavior>.
...
Implemented in
L<Perl::Mogrify::TransformerParameter::Behavior::String|Perl::Mogrify::TransformerParameter::Behavior::String>.

The most basic of behaviors, the value of the parameter will be stored
in the Transformer as a string.
...
Implemented in
L<Perl::Mogrify::TransformerParameter::Behavior::Boolean|Perl::Mogrify::TransformerParameter::Behavior::Boolean>.

The value of the parameter will be either
L<$TRUE|Perl::Mogrify::Utils/$TRUE> or
L<$FALSE|Perl::Mogrify::Utils/$FALSE>.
...
Implemented in
L<Perl::Mogrify::TransformerParameter::Behavior::Integer|Perl::Mogrify::TransformerParameter::Behavior::Integer>.
...
Implemented in
L<Perl::Mogrify::TransformerParameter::Behavior::StringList|Perl::Mogrify::TransformerParameter::Behavior::StringList>.

The values will be derived by splitting the input string on blanks.
(See L<Perl::Mogrify::Utils/"words_from_string">.) The parameter will
...
Implemented in
L<Perl::Mogrify::TransformerParameter::Behavior::Enumeration|Perl::Mogrify::TransformerParameter::Behavior::Enumeration>.

The values will be derived by splitting the input string on blanks.
(See L<Perl::Mogrify::Utils/"words_from_string">.)  Depending upon the
...
functionality and it provides a means for a C<Perl::Mogrify>
configuration program, e.g. an IDE that integrates C<Perl::Mogrify>, to
...
L<Perl::Mogrify::Transformer|Perl::Mogrify::Transformer> that takes two
parameters: the
L<Perl::Mogrify::TransformerParameter|Perl::Mogrify::TransformerParameter> that is
...
is thrown.  This allows C<Perl::Mogrify> to include that problem along
...
There are cases where a Transformer needs additional initialization beyond
...
the Transformer by the time C<initialize_if_enabled()> has been called.  It
...
=head2 C<default_maximum_transformations_per_document()>

Certain problems that a Transformer detects can be endemic to a particular
file; if there's one transformation, there's likely to be many.  A good
example of this is
L<Perl::Mogrify::Transformer::TestingAndDebugging::RequireUseStrict|Perl::Mogrify::Transformer::TestingAndDebugging::RequireUseStrict>;
...
much help to the user to report every single transformation.  If you've got
such a policy, you should override
L<default_maximum_transformations_per_document()|Perl::Mogrify::Transformer/"default_maximum_transformations_per_document()">
method to provide a limit.  The user can override this value with a
value for "maximum_transformations_per_document" in their F<.perlmogrifyrc>.

See the source code for
L<Perl::Mogrify::Transformer::ValuesAndExpressions::ProhibitMagicNumbers|Perl::Mogrify::Transformer::ValuesAndExpressions::ProhibitMagicNumbers>
and
L<Perl::Mogrify::Transformer::TestingAndDebugging::RequireUseWarnings|Perl::Mogrify::Transformer::TestingAndDebugging::RequireUseWarnings>
for examples.


=head2 C<is_safe()>

Most L<Perl::Mogrify> Policies are purely I<static>.  In other words,
...
L<Perl::Mogrify::Dynamic|Perl::Mogrify::Dynamic> for an example).  So
the C<is_safe()> method is used to indicate whether a Transformer can be
trusted to not cause mischief.  By default, C<is_safe()> returns true.
But if you are writing a Transformer that will compile or execute any of
...
You need to come up with a name for your set of transformers.  Sets of
add-on transformers are generally named C<Perl::Mogrify::I<something>>,
e.g. L<Perl::Mogrify::More|Perl::Mogrify::More>.
...
part is that this will include a list of the included transformers, with
descriptions of each.

A typical implementation will look like:

    package Perl::Mogrify::Example;
    ...
    Perl::Mogrify::Example - Policies for Perl::Mogrify that act as an example.
    ...
    into a bundle.  All of the Transformer modules contained herein will have
    ...
    Some L<Perl::Mogrify|Perl::Mogrify> transformers that will help you keep your
    ...
    =item L<Perl::Mogrify::Transformer::Documentation::Example|Perl::Mogrify::Transformer::Documentation::Example>

    Complains about some example documentation issues.  [Default severity: 3]


    =item L<Perl::Mogrify::Transformer::Variables::Example|Perl::Mogrify::Transformer::Variables::Example>
    ...
    All transformers included are in the "example" theme.  See the
    L<Perl::Mogrify|Perl::Mogrify> documentation for how to make use of this.


=head2 Themes

Users can choose which transformers to enable using themes.  You should
implement C<default_themes()> so that users can take advantage of
this.  In particular, you should use a theme named after your
distribution in all your transformers; this should match the value listed
...
output of C<perlmogrify --list-themes>.
...
Since all transformers have to go somewhere under the
C<Perl::Mogrify::Transformer::> namespace, it isn't always clear what
...
section in the POD for all of your transformers that state where the
policy comes from.  For example:

    =head1 AFFILIATION

    This policy is part of L<Perl::Mogrify::Example|Perl::Mogrify::Example>.
...
The L<Perl::Mogrify> distribution also contains a framework for testing your
Transformer.  See L<Test::Perl::Mogrify::Transformer> for the details.
...
L<Perl::Mogrify|Perl::Mogrify> distribution that will help you.  For
...
Chas. Owens has a blog post about developing in-house transformers at
L<http://svok.blogspot.com/2009/09/adding-house-transformers-to-perlmogrify.html>.

...
Perl::Mogrify::CORE_DEVELOPER - Hints for working on the Perl::Mogrify core.


=head1 DESCRIPTION

This document is a grab-bag of notes for those who are working on the
underpinnings of Perl::Mogrify. They are intended to be informative,
...
* If you're unsure of yourself, install Perl::Mogrify, then download
the source and rummage around in it.

** Subscribe to the developers' mailing list. There are instructions
in L<Perl::Mogrify/EXTENDING THE MOGRIFIER>. The F<commits> mailing list
is another good one.
...
** The Perl Mogrify subversion repository on Tigris contains a bunch of
stuff other than Perl::Mogrify. You probably do not need it all. But if
...
* Here is my "branch and checkout" cookbook:
    ...
    $ svn copy
        http://perlmogrify.tigris.org/svn/perlmogrify/trunk/distributions/Perl-Mogrify
...
required to run C<Perl::Mogrify>. The list probably depends on which
C<Perl::Mogrify> you are testing, so the following should not be
considered definitive.  You need the following in addition to B<all>
optional modules for Perl::Mogrify itself.
...
F<inc/Perl/Mogrify/BuildUtilities.pm>.
...
=head3 F<lib/Perl/Mogrify/Utils/Constants.pm>
...
=head3 F<lib/Perl/Mogrify/Command.pm>
...
=head3 F<lib/Perl/Mogrify/OptionsProcessor.pm>
...
L<Perl::Mogrify::Config|Perl::Mogrify::Config>, which is where the
action is.
...
=head3 F<lib/Perl/Mogrify/Config.pm>
...
=head3 F<lib/Perl/Mogrify/ProfilePrototype.pm>
...
exist to serve Perl::Mogrify, but if it is documented as public it may
well be in use "in the wild", either in add-ons to Perl::Mogrify or by
users of Perl::Mogrify.

Before deprecating public code, the potential deprecator must discuss
the issues on the Perl::Mogrify developers' mailing list. There are
instructions on how to subscribe to this list in
L<Perl::Mogrify/EXTENDING THE MOGRIFIER>.
...
deprecated:
    ...
    warnings::warnif(
        'deprecated',
        'Perl::Mogrify::Utils::foo() deprecated, use blah::foo() instead.',
    );

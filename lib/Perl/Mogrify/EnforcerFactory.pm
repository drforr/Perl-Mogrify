package Perl::Mogrify::EnforcerFactory;
...
use Perl::Mogrify::Exception::Configuration::NonExistentEnforcer qw< >;
...
sub create_policy {
    ...
    if ( $args{-params} ) {
        ...
        $policy_config ||=
            Perl::Mogrify::EnforcerConfig->new( $policy_short_name );
    }
...
sub _validate_policies_in_profile {
    ...
    for my $policy_name ( $profile->listed_policies() ) {
        if ( not exists $known_policies{$policy_name} ) {
            my $message = qq{Enforcer "$policy_name" is not installed.};

            if ( $errors ) {
                $errors->add_exception(
                    Perl::Mogrify::Exception::Configuration::NonExistentEnforcer->new(
                        policy  => $policy_name,
                    )
                );
            }
...
Perl::Mogrify::EnforcerFactory - Instantiates Enforcer objects.


=head1 DESCRIPTION

This is a helper class that instantiates
L<Perl::Mogrify::Enforcer|Perl::Mogrify::Enforcer> objects with the user's
...
Returns a reference to a new Perl::Mogrify::EnforcerFactory object.
...
B<-name> is the name of a L<Perl::Mogrify::Enforcer|Perl::Mogrify::Enforcer>
subclass module.  The C<'Perl::Mogrify::Enforcer'> portion of the name
...
L<Perl::Mogrify::Enforcer/"initialize_if_enabled"> invoked on it, so it
...
L<Perl::Mogrify::Enforcer|Perl::Mogrify::Enforcer> subclass that is
...
L<Perl::Mogrify::Enforcer/"initialize_if_enabled"> invoked on them, so
...
Perl::Mogrify::EnforcerFactory has a few static subroutines that are used

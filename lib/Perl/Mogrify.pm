...
sub new {
    my ( $class, %args ) = @_;
    my $self = bless {}, $class;
    $self->{_config} = $args{-config} || Perl::Mogrify::Config->new( %args );
    $self->{_stats} = Perl::Mogrify::Statistics->new();
    return $self;
}
...
sub critique {  ## no mogrify (ArgUnpacking)
    ...
    my $doc =
        blessed($source_code) && $source_code->isa('Perl::Mogrify::Document')
            ? $source_code
...
L<Perl::Mogrify::Utils::Constants/"$PROFILE_STRICTNESS_QUIET"> makes
...
Returns the L<Perl::Mogrify::Config> object that was created for or given to
this Mogrify.

=item C< statistics() >

Returns the L<Perl::Mogrify::Statistics> object that was created for this
...
Default settings for Perl::Mogrify itself can be set B<before the first named
...
=head1 THE L<Perl::Mogrify> PHILOSOPHY
...
L<Perl::Mogrify::Bangs>

L<Perl::Mogrify::Lax>

L<Perl::Mogrify::StricterSubs>

L<Perl::Mogrify::Swift>
...
L<Test::Perl::Mogrify::Progressive>
...
Adam Kennedy - For creating L<PPI>, the heart and soul of L<Perl::Mogrify>.

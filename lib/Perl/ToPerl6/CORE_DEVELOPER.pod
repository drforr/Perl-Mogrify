...
Perl::ToPerl6::CORE_DEVELOPER - Hints for working on the Perl::ToPerl6 core.


=head1 DESCRIPTION

This document is a grab-bag of notes for those who are working on the
underpinnings of Perl::ToPerl6. They are intended to be informative,
...
* If you're unsure of yourself, install Perl::ToPerl6, then download
the source and rummage around in it.

** Subscribe to the developers' mailing list. There are instructions
in L<Perl::ToPerl6/EXTENDING THE MOGRIFIER>. The F<commits> mailing list
is another good one.

** You will need to be registered as a developer before you can
actually change code. L<http://perlmogrify.tigris.org/> only lets you
...
** The Perl ToPerl6 subversion repository on Tigris contains a bunch of
stuff other than Perl::ToPerl6. You probably do not need it all. But if
...
* Here is my "branch and checkout" cookbook:
    ...
    $ svn copy
        http://perlmogrify.tigris.org/svn/perlmogrify/trunk/distributions/Perl-ToPerl6
        http://perlmogrify.tigris.org/svn/perlmogrify/branches/rt25046
        -m "Put your comment here" --username your_username

    $ svn checkout http://perlmogrify.tigris.org/svn/perlmogrify/branches/rt25046
...
required to run C<Perl::ToPerl6>. The list probably depends on which
C<Perl::ToPerl6> you are testing, so the following should not be
considered definitive.  You need the following in addition to B<all>
optional modules for Perl::ToPerl6 itself.
...
F<inc/Perl/ToPerl6/BuildUtilities.pm>.
...
Perlmogrify handles global configuration items and command line options
...
=head3 F<lib/Perl/ToPerl6/Utils/Constants.pm>
...
=head3 F<lib/Perl/ToPerl6/Command.pm>
...
=head3 F<lib/Perl/ToPerl6/OptionsProcessor.pm>
...
between the physical F<.perlmogrifyrc> file and
L<Perl::ToPerl6::Config|Perl::ToPerl6::Config>, which is where the
action is.
...
=head3 F<lib/Perl/ToPerl6/Config.pm>
...
=head3 F<lib/Perl/ToPerl6/ProfilePrototype.pm>
...
=head3 F<examples/perlmogrifyrc>
...
exist to serve Perl::ToPerl6, but if it is documented as public it may
well be in use "in the wild", either in add-ons to Perl::ToPerl6 or by
users of Perl::ToPerl6.

Before deprecating public code, the potential deprecator must discuss
the issues on the Perl::ToPerl6 developers' mailing list. There are
instructions on how to subscribe to this list in
L<Perl::ToPerl6/EXTENDING THE MOGRIFIER>.
...
deprecated:
    ...
    warnings::warnif(
        'deprecated',
        'Perl::ToPerl6::Utils::foo() deprecated, use blah::foo() instead.',
    );
